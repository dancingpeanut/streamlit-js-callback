...
BASIC_EXAMPLE_FILE = ROOT_DIRECTORY / "streamlit_js_callback" / "example.py"
...
def test_should_render_template(page: Page):
    frame_0 = page.frame_locator(
        'iframe[title="streamlit_js_callback\\.streamlit_js_callback"]'
    ).nth(0)
...
def test_should_change_iframe_height(page: Page):
    frame = page.frame_locator('iframe[title="streamlit_js_callback\\.streamlit_js_callback"]').nth(1)

    expect(frame.get_by_text("Hello, Streamlit!")).to_be_visible()

    locator = page.locator('iframe[title="streamlit_js_callback\\.streamlit_js_callback"]').nth(1)
